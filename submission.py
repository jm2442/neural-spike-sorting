...
if __name__ == "__main__":
    ...
    if plot_on:
        ...
        visuals.filter_and_detection(x_start, x_end, time, d, time_test=[], index_train=idx_train, index_test=[], filtered_data=filt_d, smoothed_data=smth_d, smoothed_threshold=smth_thresh, edo_data=edo_d, edo_threshold=edo_thresh, training=False)
        ...
        if clf_type == 'MLP':
            visuals.MLP(input_samples, pred_lbl, d_samp_window)
        elif clf_type == 'KNN':
            visuals.KNN(input_samples, pred_lbl, d_samp_window)
